 ...
 * DESCRIPCIÓN: Control de velocidad suavizado para una lectura natural y pausada.
 */
...
function typeEffect(text, container, callback) {
    ...
    }, 45); // Velocidad de la intro hacker pulida
}
...
function runCommand(tipo) {
    ...
    function typeWriter() {
        if (i < text.length) {
            ...
            i++;
            setTimeout(typeWriter, 55); // CORREGIDO: Bajado a 55ms para que se lea perfectamente mientras aparece
        } else {
            isTyping = false;
